...
class BuyerProfileToolsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (!Get.isRegistered<BuyerProfileController>()) Get.put(BuyerProfileController());
    ...
    return GetBuilder<BuyerProfileController>(
      ...
      builder: (_) => Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(_titleKey.tr, style: AppTextStyle.heading2.copyWith(color: Colors.white)),
          centerTitle: true,
        ),
...
class _EditProfile extends StatefulWidget {
  ...
  @override State<_EditProfile> createState() => _EditProfileState();
}
class _EditProfileState extends State<_EditProfile> {
  ...
  @override void dispose() { first.dispose(); last.dispose(); email.dispose(); phone.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => ListView(
    padding: const EdgeInsets.all(16), children: [
      ...
      SizedBox(height: 50, child: ElevatedButton(
        onPressed: () => widget.controller.updateProfile(firstName: first.text, lastName: last.text, email: email.text, phone: phone.text),
  ...
  Widget _field(TextEditingController c, String label, {TextInputType? keyboard}) => Padding(
    padding: const EdgeInsets.only(bottom: 12), child: TextField(controller: c, keyboardType: keyboard,
      decoration: InputDecoration(labelText: label, filled: true, fillColor: Get.isDarkMode ? AppColor.darkCard : Colors.white,
...
class _Addresses extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => ListView(
    padding: const EdgeInsets.all(16), children: [
      ...
      }, icon: const Icon(Icons.map_outlined), label: Text('pick_address'.tr)),
      const SizedBox(height: 8),
      OutlinedButton.icon(onPressed: () => _add(context), icon: const Icon(Icons.add), label: Text('add_new_address'.tr)),
    ],
  );
  Future<void> _add(BuildContext context) async {
    final title = TextEditingController(), details = TextEditingController();
    await showDialog(context: context, builder: (_) => AlertDialog(
      title: Text('add_new_address'.tr), content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: title, decoration: InputDecoration(labelText: 'address_title'.tr)),
        TextField(controller: details, decoration: InputDecoration(labelText: 'detected_address'.tr)),
      ]), actions: [TextButton(onPressed: Get.back, child: Text('cancel'.tr)), ElevatedButton(
        onPressed: () { Get.back(); controller.saveAddress({'title': title.text.trim(), 'details': details.text.trim(), 'is_default': controller.addresses.isEmpty}); },
...
class _Wallet extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [
    Card(color: AppColor.primaryColor, child: Padding(padding: const EdgeInsets.all(22), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('sham_cash_balance'.tr, style: AppTextStyle.bodyMedium.copyWith(color: Colors.white)), const SizedBox(height: 8),
      Text((controller.wallet['balance'] ?? controller.wallet['data']?['balance'] ?? '0').toString(), style: AppTextStyle.heading1.copyWith(color: Colors.white)),
    ]))),
    const SizedBox(height: 12),
    OutlinedButton.icon(onPressed: () => _requestDeposit(context), icon: const Icon(Icons.add), label: Text('wallet_add_balance'.tr)),
    const SizedBox(height: 20), Text('transaction_history'.tr, style: AppTextStyle.heading3),
    ...controller.walletHistory.map((item) => ListTile(title: Text(item['description']?.toString() ?? item['type']?.toString() ?? ''), subtitle: Text(item['created_at']?.toString() ?? ''), trailing: Text(item['amount']?.toString() ?? ''))),
    const SizedBox(height: 18), Text('wallet_deposit_requests'.tr, style: AppTextStyle.heading3),
    ...controller.depositRequests.map((item) => ListTile(title: Text('${item['amount'] ?? ''}'), subtitle: Text(item['created_at']?.toString() ?? ''), trailing: Text('${item['status'] ?? ''}'))),
  ]);
  ...
  Future<void> _requestDeposit(BuildContext context) async {
    ...
    await showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: amount, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'wallet_amount'.tr)),
        TextField(controller: reference, decoration: InputDecoration(labelText: 'wallet_reference'.tr)),
      ]),
      actions: [
        TextButton(onPressed: Get.back, child: Text('cancel'.tr)),
        ElevatedButton(onPressed: () { Get.back(); final value = double.tryParse(amount.text.trim()); if (value != null && value > 0) controller.requestWalletDeposit(value, reference.text); }, child: Text('send_request'.tr)),
      ],
    ));
  }
}
...
class _Notifications extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [
    Align(alignment: AlignmentDirectional.centerEnd, child: TextButton(
      onPressed: () {
        for (final type in ['order_status', 'store_offers', 'favorite_products', 'messages']) {
          controller.notificationSettings[type] = true;
        }
        controller.update();
        controller.saveNotificationSettings();
      },
      child: Text('enable_all_notifications'.tr),
    )),
    ...['order_status', 'store_offers', 'favorite_products', 'messages'].map((type) => SwitchListTile(
      title: Text(type.tr), value: controller.notificationSettings[type] ?? true,
      onChanged: (value) { controller.notificationSettings[type] = value; controller.update(); controller.saveNotificationSettings(); },
    )),
  ]);
}
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final conversations = snapshot.data!.docs.toList()
          ..sort((a, b) {
            final aTime = (a.data()['last_time'] as Timestamp?)?.millisecondsSinceEpoch ?? 0;
            final bTime = (b.data()['last_time'] as Timestamp?)?.millisecondsSinceEpoch ?? 0;
