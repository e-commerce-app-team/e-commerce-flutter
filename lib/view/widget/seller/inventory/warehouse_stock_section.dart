...
class WarehouseStockSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (warehouses.isEmpty) return _EmptyWarehouses();
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Text(
              'warehouse_distribution_info'.tr,
              style: AppTextStyle.labelSmall.copyWith(color: AppColor.infoDark, fontSize: 11),
            ),
          ),
        ]),
      ),
      const SizedBox(height: 12),
      ...warehouses.where((w) => w.isActive).map((w) => _WarehouseRow(
        warehouse: w,
      ...
      Row(
        ...
        children: [
          Text('total_qty_label'.tr,
              style: AppTextStyle.labelLarge.copyWith(fontSize: 13)),
          AnimatedContainer(
            duration: const Duration(milliseconds: 200),
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
            decoration: BoxDecoration(
              color: _total > 0 ? AppColor.primarySurface : AppColor.secondBackground,
              borderRadius: BorderRadius.circular(10),
              border: Border.all(
                color: _total > 0
            ...
            child: Text(
              ...
              style: AppTextStyle.price.copyWith(
                fontSize: 14,
                color: _total > 0 ? AppColor.primaryColor : AppColor.greyLight,
              ),
            ),
          ),
        ],
...
class _WarehouseRow extends StatelessWidget {
  ...
  const _WarehouseRow({required this.warehouse, required this.qty, required this.onChanged});

  @override
  Widget build(BuildContext context) {
    final isBranch = warehouse.type == 'branch';
    final qtyInt = int.tryParse(qty) ?? 0;
    ...
    return Padding(
      ...
      child: Row(children: [
        Container(
          width: 38, height: 38,
          decoration: BoxDecoration(
            color: isBranch ? AppColor.infoLight : AppColor.primarySurface,
            borderRadius: BorderRadius.circular(10),
          ),
          child: Icon(
            isBranch ? Icons.store_outlined : Icons.warehouse_outlined,
            size: 17,
            color: isBranch ? AppColor.info : AppColor.primaryColor,
          ),
        ),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Container(
                ...
                child: Text(
                  isBranch ? 'branch_type'.tr : 'warehouse_type'.tr,
        ...
        SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: '0',
              hintStyle: AppTextStyle.inputHint.copyWith(fontSize: 13),
              filled: true,
              fillColor: qtyInt > 0 ? AppColor.primarySurface : AppColor.secondBackground,
...
class _EmptyWarehouses extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      const Icon(Icons.warning_amber_rounded, size: 17, color: AppColor.warning),
      const SizedBox(width: 10),
      Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('no_warehouses_title'.tr,
              style: AppTextStyle.labelLarge.copyWith(color: AppColor.warningDark, fontSize: 13)),
          Text('no_warehouses_sub'.tr,
